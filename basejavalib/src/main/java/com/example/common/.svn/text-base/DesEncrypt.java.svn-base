...
public class DesEncrypt {
	...
	static public byte[] Des(byte[] key, byte[] data, int offset, int length,
			boolean isEncrypt) {
		if (key.length != 8)
			return null;
		...
		while (position < offset + length) {
			int nCopyBytes = 8;
			if (offset + length - position < 8) {
				// 不足8个字节
				nCopyBytes = offset + length - position;
				for (int ii = 0; ii < 8; ii++)
					source[ii] = 0;
			}

			for (int ii = 0; ii < nCopyBytes; ii++)
				source[ii] = data[position + ii];

			if (isEncrypt)
				target = des.Des(key, source, 1);
			else
				target = des.Des(key, source, 0);
			...
			for (int ii = 0; ii < 8; ii++)
				targetData[position + ii - offset] = target[ii];
